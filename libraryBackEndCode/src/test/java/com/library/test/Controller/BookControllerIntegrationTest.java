...
public class BookControllerIntegrationTest {
	...
	public void getBookListTest() throws Exception {
		MvcResult result = mockMvc
				.perform(get("/bookList").content(MediaType.APPLICATION_JSON_VALUE))
	...
	public void getBorrowedBookListTest() throws Exception {
		//userid mocked to 1
		MvcResult result = mockMvc
				.perform(get("/1/borrowedBookList").content(MediaType.APPLICATION_JSON_VALUE))
				.andExpect(status().isOk()).andReturn();
		String resultContent = result.getResponse().getContentAsString();
		Assert.assertTrue(prop.getProperty("book.return"), resultContent.contains("books"));
	...
	private void bookSuccessfulBorrow() throws Exception{
		MvcResult result = mockMvc
				.perform(post("/1/borrow/2").content(MediaType.APPLICATION_JSON_VALUE))
				.andReturn();
		String resultContent = result.getResponse().getContentAsString();
	...
	private void bookNoTExistsTest() throws Exception {
		// userid mocked to 1
		MvcResult result = mockMvc
				.perform(post("/1/borrow/3").content(MediaType.APPLICATION_JSON_VALUE))
				.andReturn();
		String resultContent = result.getResponse().getContentAsString();
	...
	private void borrowBookLimitExceededTest() throws Exception {
		//userid mocked to 1
		mockMvc
		.perform(post("/1/borrow/1").content(MediaType.APPLICATION_JSON_VALUE))
		.andReturn();
		MvcResult result = mockMvc
				.perform(post("/1/borrow/1").content(MediaType.APPLICATION_JSON_VALUE))
				.andReturn();
